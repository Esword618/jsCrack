# author:Ajian
# contact:jyj349559953@qq.com
# datetime:2020/12/8 18:30
# software:PyCharm
...
# ASCII编码实际上可以被看成是UTF-8编码的一部分,encode默认编码是utf-8
# btoa() 函数能够从二进制数据“字符串”创建一个base-64编码的ASCII字符串。
# var j_key = '.' + hex_md5(btoa(data.key + data.value).replace(/=/g, ''))

def btoa(key, value):
    data = key + value
    data = base64.b64encode(data.encode('ascii')).decode('utf8').replace('=', '')
    # print(data)
    return data


def hex_md5(datas):
    md5 = hashlib.md5(datas)
    j_key = md5.hexdigest()
    # print(j_key)
    return j_key
...
def nums(list_1):
    ...
    str_num = ''
    for m in list_2:
        str_num=str_num+str(m[0])
    sum_list.append(int(str_num))
    print(sum_list)
    print(str_num)

def spider(pn):
    url = 'http://match.yuanrenxue.com/api/match/4?page={}'.format(pn)
    headers = {
    ...
    key = dict['key']
    value = dict['value']
    datas = btoa(key, value)
    j_key = hex_md5(datas.encode())
    # print(j_key)
    ...
    # print(td_list)
    for td in td_list:
        list = re.findall('src="(data:.*?)" class="(img_number .*?)" style="left:(.*?)px"',td)
        # print(list)
        list_1 = []
        for i in list:
            if j_key not in i[1]:
                num = int(img_dict[i[0]])
                left_num = int(float(i[2])/11.5)
                list_1.append([num,left_num])
                # print(num,left_num)
        # print(list_1)
        nums(list_1)
